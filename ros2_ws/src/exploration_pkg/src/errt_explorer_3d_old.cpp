...
private:
    ...
    void explorationLoop() {
        ...
        if (!found_frontier) return;
            ...
            // Force slight forward push into cave
        double forward_dist = 2.0;

        double new_x = current_x_ + forward_dist * cos(current_yaw_);
        double new_y = current_y_ + forward_dist * sin(current_yaw_);
        double new_z = current_z_;
        geometry_msgs::msg::PoseStamped msg;
        msg.header.stamp = now();
        msg.header.frame_id = "world";
        msg.pose.position.x = new_x;
        msg.pose.position.y = new_y;
        msg.pose.position.z = new_z;

        tf2::Quaternion q;
        q.setRPY(0, 0, atan2(new_y - current_y_,
                             new_x - current_x_));
